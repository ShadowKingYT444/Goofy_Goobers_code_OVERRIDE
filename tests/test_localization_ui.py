...
class FirmwareSafetyInvariantTests(unittest.TestCase):
    ...
    def test_port_nine_is_exclusively_the_slider_motor(self):
        ...
        self.assertIn("slider_right.move_voltage(voltage)", cascade)
        self.assertIn("slider_left.move_voltage(voltage)", cascade)
        ...
        self.assertIn("kMinimumDownwardPidMv = 2500.0", cascade)
    ...
    def test_claw_and_wrist_controls_match_current_mapping(self):
        ...
        self.assertNotIn("update_wrist_hold", main)
        self.assertNotIn("kWristDefaultKp", main)
        ...
        self.assertIn("pros::Task arm_lowering_task", main[auton_dispatch:route_dispatch])
        self.assertIn("pros::delay(300);", main[auton_dispatch:route_dispatch])
        self.assertIn("pros::delay(1000);", startup)
        self.assertIn("pros::delay(200);", startup)
        self.assertIn("arm_lowering_task.join()", startup)
        self.assertNotIn("pros::delay(100);", startup)
        self.assertIn("claw_arm.move(kAutonArmLoweringPower)", main[auton_dispatch:route_dispatch])
        self.assertIn("claw_arm.move(0)", main[auton_dispatch:route_dispatch])
        self.assertIn("horizontal_odom.reset_position()", main[auton_dispatch:route_dispatch])
        self.assertIn("ARM_HOME relative_deg", startup)

    def test_left_x_starts_exact_far_goal_auton(self):
        main = (ROOT / "src" / "main.cpp").read_text(encoding="utf-8")
        autons = (ROOT / "src" / "autons.cpp").read_text(encoding="utf-8")
        self.assertIn("start_toggle_far_goal_auton();", main)
        self.assertIn("E_CONTROLLER_DIGITAL_LEFT", main)
        listener_start = main.index("single owner of Left+X")
        listener_end = main.index("auton selector", listener_start)
        listener = main[listener_start:listener_end]
        self.assertIn("E_CONTROLLER_DIGITAL_X", listener)
        self.assertNotIn("E_CONTROLLER_DIGITAL_A", listener)
        self.assertIn("hotkey_left_x_listener", listener)
    ...
    def test_two_cup_red_selector_and_phase_one_route_are_explicit(self):
        ...
        self.assertIn("bool left_press_had_x = false", main)
        self.assertIn("if (left && x_held) left_press_had_x = true", main)
        self.assertIn("if (!left_press_had_x && selectable) select_red_auton(+1)", main)
        ...
        self.assertIn("kPhase2GoalClearanceIn = 11.0", config)
        self.assertIn("kStage1ScoreLoweringDeg = 85.0", config)
        ...
        self.assertIn("kStage2ScoreLoweringDeg = 85.0", config)
        self.assertIn("kSecondStackApproachPower = 60", config)
        self.assertIn("kSecondStackSlowPower = 38", config)
        self.assertIn("kFirstCupLiftStage = 2", config)
        self.assertIn("kSecondCupLiftStage = 3", config)
        ...
        self.assertIn("kTestStopAfterPhase = 4", config)
        ...
        self.assertIn("constexpr double kPneumaticGrabLeadIn = 8.5", route)
        self.assertIn("constexpr double kPneumaticGrabLeadIn = 7.0", route)
        ...
        self.assertIn("final 8.5 inches based on the latest loaded test", route)
        ...
        self.assertIn("first_score_drop_not_reached", route)
        self.assertIn("second_score_drop_not_reached", route)
        ...
        second_stack_turn = route.index(
            "path2_chain_turn(kSecondStackPickupHeadingDeg, false, 2.5)"
        )
        ...
        self.assertIn("path2_fast_turn(90.0, false, 1.5)", route)
        self.assertIn("cascade_lift::clear_fault()", route)
        self.assertIn("path2_home_lift_at_bottom", route)
        self.assertIn("PATH2_LIFT_HOME result=success", route)
        self.assertIn("cascade_lift::initialize_at_rest()", route)
        self.assertIn("abort=lift_bottom_home_failed", route)
        ...
        self.assertIn("passive braking until retuning", route)
        self.assertIn("if (!path2_fast_turn(180.0)) return false;", route)
        self.assertIn("test_stop=phase_4_after_second_stack_score", route)
    ...
    def test_public_go_to_pose_is_curved_bounded_and_heading_complete(self):
        ...
        self.assertIn("if (!stationary_for_navigation_init()) return false;",
                      init_block)
